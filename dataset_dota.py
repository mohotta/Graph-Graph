...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        cx_1, cy_1, cx_2, cy_2 = all_bbox[:, :, 0], all_bbox[:, :, 1], all_bbox[:, :, 2], all_bbox[:, :, 3]
        all_obj_centers = torch.cat((cx_1.unsqueeze(2), cy_1.unsqueeze(2), cx_2.unsqueeze(2), cy_2.unsqueeze(2)), 2).float()
        ...
        for i in range(all_feat.shape[0]):
            ...
            obj_centers[:, 0:2] = obj_centers[:, 0:2] / frame_stats[i]  # normalize with frame height and width
            obj_centers[:, 2:] = obj_centers[:, 2:] / frame_stats[i]
