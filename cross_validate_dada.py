...
parser.add_argument("--dataset_path", type=str, default="data/dada/obj_feat", help="Path to extracted objects data")
parser.add_argument("--img_dataset_path", type=str, default="data/dada/i3d_feat", help="Path to I3D feature data")
parser.add_argument("--toas_files_path", type=str, default="data/dada/toas", help="Path to frame of accidents feature data")
parser.add_argument("--obj_mapping_file", type=str, default="data/dada/obj_idx_to_labels.json",
                    help="path to object label mapping file")
parser.add_argument("--split_path", type=str, default="splits_dada/", help="Path to train/test split")
...
def test_model(epoch, model, test_dataloader, fold):
    ...
    if avg_prec > best_ap:
        best_ap = avg_prec
        os.makedirs("model_checkpoints/dada", exist_ok=True)
        torch.save(model.state_dict(), f"model_checkpoints/dada/{model.__class__.__name__}_{fold+1}_{epoch+1}.pth")
        print(f"Saved the model checkpoint - model_checkpoints/dada/{model.__class__.__name__}_{fold+1}_{epoch+1}.pth")
